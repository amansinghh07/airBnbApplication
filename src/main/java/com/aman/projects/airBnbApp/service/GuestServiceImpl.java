...
import com.aman.projects.airBnbApp.repository.GuestRepository;
...
public class GuestServiceImpl implements GuestService {
    ...
    public void deleteGuest(Long guestId) {
        ...
        User user = getCurrentUser();
        if(!user.equals(guest.getUser())) throw new AccessDeniedException("You are not the owner of this guest");
        boolean usedInBooking = bookingRepository.existsByGuests_Id(guestId);

        if (usedInBooking) {
            throw new IllegalStateException("Guest cannot be deleted because it is used in bookings");
        }
